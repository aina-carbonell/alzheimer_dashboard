...
export const BoxPlotChart = ({ variable, title, description }: BoxPlotChartProps) => {
    ...
    const scaleY = (value: number) => { return ((value - globalMin) / range) *200;};
    const boxWidth = 60;
                ...
                <div className="overflow-x-auto">
                    <svg width={chartWidth} height={280} classNAme="mx-auto">
                        {[0, 0.25, 0.5, 0.75, 1].map((tick, i) => {
                            ...
                            return (
                                <g key={i}>
                                    ...
                                    <text x={40} y={y + 4} textAnchor="end" fontSize="10" fill="#hsl(var(--foreground))">
                        ...
                        {boxPlotData.map((data, index) => {
                            ...
                            return (
                                <motion.g key={data.diagnosis} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                                    ...
                                    <line x1={x - 15} y1={medianY} x2={x + 15} y2={medianY} stroke={data.color} strokeWidth={2} />
                                    <rect x={x - boxWidth / 2} y={q3Y} width={boxWidth} height={q1Y - q3Y} fill={data.bgColor} stroke={data.color} strokeWidth={2} />
                                    {data.outliers.map((outlier, i) => {
                                        <circle key={i} cx={x} cy={230 - scaleY(outlier)} r={4} fill="transparent" stroke={data.color} strokeWidth={2} />
                                    })}
                                    <text x={x} y={255} textAnchor="middle" fontSize="11" fill="hsl(var(--foreground))" fontWeight={500}>{data.diagnosis.split(' ')[0]}</text>
                                    {data.diagnosis.split(' ').length > 1 && (<text x={x} y={268} textAnchor="middle" fontSize="11" fill="hsl(var(--foreground))" fontWeight={500}>{data.diagnosis.split(' ').slice(1).join(' ')}</text>)}
                                </motion.g>
                            );
                        })}
                    </svg>
                    ...
                    {boxPlotData.map(data => (
                        <div key={data.diagnosis} className="p-3 rounded-lg bg-muted/30 text-center">
                            ...
                            <div className="text-sm font-medium" style={{ color: data.color }}>
                                Mediana: {variable === 'mmse' ? data.median.toFixed(0) : data.median.toFixed(3)}
                            </div>
                        </div>
                    ))}
                </div>
